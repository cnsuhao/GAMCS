...
public:
    ...
    enum StFlag {SAVED, NEW, MODIFIED};   /**< storage status of a state */
...
private:
    ...
    void LinkStates(struct cs_State *, Agent::EnvAction, Agent::Action, struct cs_State *);  /**< link two states in memory with specfic exact and action */
    std::vector<Agent::Action> BestActions(const struct cs_State *, const std::vector<Agent::Action>&);           /**< find the best action of a state */
    ...
    struct cs_Action *Act2Struct(Agent::Action, const struct cs_State *) const;                  /**< find the Agent::Action struct address according to identity */
    struct cs_EnvAction *Eat2Struct(Agent::EnvAction, const struct cs_State *) const;              /**< find the exact strut address according to identity */
    struct cs_State *StateByEatAct(Agent::EnvAction, Agent::Action, const struct cs_State *) const;      /**< find the following state according to exact and Agent::Action*/
