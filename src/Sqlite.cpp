...
int Sqlite::connect()
{
	...
	int ret = sqlite3_open_v2(db_name.c_str(), &db_con,
...
Agent::State Sqlite::stateByIndex(unsigned long index) const
{
	...
	sqlite3_stmt *stmt;
	int ret = sqlite3_prepare_v2(db_con, query_str, 0, &stmt, 0);
	if (ret != SQLITE_OK)
	{
	...
	{
		while ((ret = sqlite3_step(stmt)) != SQLITE_DONE)	// only iterate once
		{
			switch (ret)
			{
			case SQLITE_BUSY:
				fprintf(stderr, "stateByIndex - sqlite busy, sleep 20ms\n");
				usleep(20000);
				break;
			case SQLITE_ERROR:
				fprintf(stderr, "stateByIndex - step error: %s\n",
						sqlite3_errmsg(db_con));
				break;
			case SQLITE_ROW:
...
struct State_Info_Header *Sqlite::getStateInfo(Agent::State st) const
{
	...
	int ret = sqlite3_prepare_v2(db_con, query_string, 0, &stmt, 0);
	if (ret != SQLITE_OK)
	{
	...
	else
	{
		while ((ret = sqlite3_step(stmt)) != SQLITE_DONE)
		{
			...
			{
			case SQLITE_BUSY:
				fprintf(stderr, "getStateInfo - sqlite busy, sleep 20ms!\n");
				usleep(20000);
				break;
			case SQLITE_ERROR:
				fprintf(stderr, "getStateInfo - step error: %s\n",
						sqlite3_errmsg(db_con));
				break;
			case SQLITE_ROW:	// find the state
...
bool Sqlite::hasState(Agent::State st) const
{
	...
	else
	{
		while ((ret = sqlite3_step(stmt)) != SQLITE_DONE)
		{
			...
			{
			case SQLITE_BUSY:
				fprintf(stderr, "hasState - sqlite busy, sleep 20ms!\n");
				usleep(20000);
				break;
			case SQLITE_ERROR:
				fprintf(stderr, "hasState - step error: %s\n",
						sqlite3_errmsg(db_con));
				break;
			case SQLITE_ROW:
				result = true;
...
void Sqlite::addStateInfo(const struct State_Info_Header *sthd)
{
	...
	sqlite3_stmt *stmt;
	int ret = sqlite3_prepare_v2(db_con, stmt_buf, 0, &stmt, 0);
	if (ret != SQLITE_OK)
	{
...
struct Memory_Info *Sqlite::getMemoryInfo() const
{
	...
	else
	{
		while ((ret = sqlite3_step(stmt)) != SQLITE_DONE)	// only iterate once
		{
			...
			{
			case SQLITE_BUSY:
				fprintf(stderr, "getMemoryInfo - sqlite busy, sleep 20ms\n");
				usleep(20000);
				break;
			case SQLITE_ERROR:
				fprintf(stderr, "getMemoryInfo - step error: %d, %s\n", ret,
						sqlite3_errmsg(db_con));
				break;
			case SQLITE_ROW:
				memif = (struct Memory_Info *) malloc(
