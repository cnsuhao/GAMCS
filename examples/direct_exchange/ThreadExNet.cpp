 ...
 */
int ThreadExNet::send(int fromid, int toid, void *buffer, size_t buf_size)
{
 ...
 */
int ThreadExNet::recv(int toid, int fromid, void *buffer, size_t buf_size)
{
 ...
 */
std::set<int> ThreadExNet::getNeighbours(int id) const
{
    std::set<int> neighs;
    neighs.clear();
...
void ThreadExNet::addMember(int mem)
{
...
void ThreadExNet::addNeighbour(int mem, int neb)
{
...
void ThreadExNet::removeMember(int mem)
{
    // check if exists
    if (members.find(mem) == members.end())    // not found
    {
...
void ThreadExNet::removeNeighbour(int mem, int neighbour)
{
    // check if member exists
    if (members.find(mem) == members.end())    // not found
    {
...
bool ThreadExNet::checkNeighbourShip(int from, int to) const
{
 ...
 * \param tf file name
 */
void ThreadExNet::loadTopoFromFile(char *tf)
{
    // check file type
    ...
    if (strcmp(ext, "exnet") == 0)    // *.exnet
    {
        ...
        while (fgets(line, 4096, topofs))    // read till end
        {
            ...
            if (strcmp(const_cast<char *>(p), "\n") == 0) continue;     // newline char
 ...
 */
void ThreadExNet::dumpTopoToFile(char *tf) const
{
    // check file type
    char *ext = strrchr(tf, '.');    // find the dot
    ext++;
...
int ThreadExNet::wrapInc(int ptr)
{
    int nptr;
...
int ThreadExNet::wrapDec(int ptr)
{
    int nptr;
    if (ptr == 0)
